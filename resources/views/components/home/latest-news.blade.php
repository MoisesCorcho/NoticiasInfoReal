...
<section id="latest-news">
    <div class="flex items-center justify-between mb-6 border-b border-white/10">
        <h2 class="text-xl font-bold uppercase text-white relative inline-block py-2">
            Últimas Noticias
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach ($articles as $article)
            <div class="bg-[#18181C] rounded-lg overflow-hidden group border border-white/10 flex flex-col">
                <a href="{{ route('article.show', $article->slug) }}" class="block">
                    <div class="relative h-48 w-full overflow-hidden">
                        @if ($article->featured_image_url)
                            ...
                            >
                        @else
                            <div class="absolute inset-0 h-full w-full bg-[#333233] flex items-center justify-center text-gray-500">
                ...
                <div class="p-4 flex flex-col flex-grow">
                    ...
                    <h3 class="text-lg font-bold text-white mt-2 leading-snug">
                        <a href="{{ route('article.show', $article->slug) }}" class="hover:underline">
                            {{ $article->title }}
                        </a>
                    </h3>
                    <p class="text-gray-400 text-sm mt-2 line-clamp-3 flex-grow">
                        {{ $article->excerpt }}
                    </p>
