...
<article {{ $attributes->merge(['class' => 'flex flex-col md:flex-row gap-6 bg-[#18181C] p-4 rounded-lg border border-white/10 transition-colors hover:bg-[#333233]']) }}>
    @if($imageUrl)
    ...
    @else
        <a href="{{ route('article.show', $article->slug) }}" class="shrink-0 md:w-1/3">
             <div class="w-full h-48 md:h-full object-cover rounded-lg bg-[#333233] flex items-center justify-center text-gray-500">Sin Imagen</div>
        </a>
    @endif
    <div class="flex-1">
        <div class="flex items-center gap-2 mb-2">
            ...
            @if($article->published_at)
                <span class="text-gray-500 text-xs">{{ $article->published_at->translatedFormat('d M, Y') }}</span>
            @endif
        </div>
