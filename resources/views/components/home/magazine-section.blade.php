            ...
            <div class="group relative">
                <a href="{{ route('article.show', $mainArticle->slug) }}" class="block">
                    <div class="relative aspect-video w-full overflow-hidden rounded-lg">
                        ...
                        <span
                            class="absolute top-3 left-3 inline-block bg-red-primary text-white text-[10px] font-bold px-1.5 py-0.5 rounded z-10">
                ...
                <div class="mt-4">
                    <h3
                        class="text-xl md:text-2xl font-bold text-white [html[data-theme=light]_&]:text-gray-900 leading-tight group-hover:text-red-primary transition-colors duration-300">
                        <a href="{{ route('article.show', $mainArticle->slug) }}">{{ $mainArticle->title }}</a>
                    </h3>
                    <p
                        class="text-gray-400 [html[data-theme=light]_&]:text-gray-600 text-sm mt-2 line-clamp-3 transition-colors duration-200">
                        {{ $mainArticle->excerpt ?? Str::limit(strip_tags($mainArticle->content), 150) }}
                    </p>
                    <div
                        class="text-gray-500 [html[data-theme=light]_&]:text-gray-600 text-xs mt-3 transition-colors duration-200">
            ...
            <div class="flex flex-col space-y-5">
                @forelse ($asideArticles as $article)
                    <div class="group flex items-center gap-4">
                        ...
                        <div>
                            <span class="inline-block text-red-primary text-[10px] font-bold mb-1">
                                {{ $article->category->name }}
                            </span>
                            <h4
                                class="text-md font-semibold text-white [html[data-theme=light]_&]:text-gray-900 leading-tight group-hover:text-red-primary transition-colors duration-300 line-clamp-2">
                                <a href="{{ route('article.show', $article->slug) }}">{{ $article->title }}</a>
                            </h4>
                            <span
                                class="text-gray-500 [html[data-theme=light]_&]:text-gray-600 text-xs mt-1 transition-colors duration-200">{{ $article->published_at->translatedFormat('j M, Y') }}</span>
                        </div>
                    </div>
                @empty
                    <p class="text-gray-500 [html[data-theme=light]_&]:text-gray-600 transition-colors duration-200">No hay
                        artículos adicionales en esta sección.</p>
                @endforelse
            </div>
        </div>
    @else
        <p class="text-gray-500 [html[data-theme=light]_&]:text-gray-600 transition-colors duration-200">No hay artículos
            disponibles para "{{ $title }}".</p>
    @endif
</section>