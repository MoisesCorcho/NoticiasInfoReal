        ...
        <li>
            <div x-data="{ open: false }" class="relative">
                <div class="flex items-center justify-between">
                    <a href="{{ route('category.show', $category->slug) }}" class="block py-1 text-gray-300 [html[data-theme=light]_&]:text-gray-700 hover:text-red-primary transition-colors">
                        {{ $category->name }}
                    </a>
                    @if ($category->children->isNotEmpty())
                        <button @click="open = !open" class="p-1 text-gray-400 [html[data-theme=light]_&]:text-gray-600 hover:text-red-primary focus:outline-none transition-colors">
