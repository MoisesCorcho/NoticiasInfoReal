...
<header x-data="{ mobileMenuOpen: false, searchOpen: false }"
    class="bg-[#101014] [html[data-theme=light]_&]:bg-white sticky top-0 z-50 border-b border-white/10 [html[data-theme=light]_&]:border-gray-200 shadow-[0_18px_45px_rgba(0,0,0,0.45)] [html[data-theme=light]_&]:shadow-sm transition-colors duration-200">
    ...
    <div class="max-w-screen-xl mx-auto px-4 relative flex items-center justify-between h-20">
        ...
        <div class="flex-1 flex justify-end items-center gap-2">
            ...
            <button id="theme-toggle"
                class="p-3 text-red-primary hover:text-white [html[data-theme=light]_&]:hover:text-red-primary focus:outline-none transition-colors duration-200"
            ...
            <button @click="mobileMenuOpen = true"
                class="p-3 text-gray-400 [html[data-theme=light]_&]:text-gray-600 hover:text-white [html[data-theme=light]_&]:hover:text-gray-900 focus:outline-none bg-[#333233] [html[data-theme=light]_&]:bg-gray-100 hover:bg-red-primary rounded-md transition-colors duration-200"
                ...
                @foreach ($mainCategories as $category)
                    <div class="static group h-full">
                        ...
                        @if ($category->articles->isNotEmpty())
                            <div
                                class="pointer-events-none invisible opacity-0 group-hover:visible group-hover:opacity-100 group-hover:pointer-events-auto transition-all duration-200">
                                <div class="absolute top-full left-0 right-0">
                                    <div
                                        class="w-full bg-[#333233] [html[data-theme=light]_&]:bg-white border border-white/10 [html[data-theme=light]_&]:border-gray-200 shadow-2xl rounded-xl overflow-hidden z-40 p-4 transition-colors duration-200">
                                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                            @foreach ($category->articles as $article)
                                                <a href="{{ route('article.show', $article->slug) }}"
                                                    class="group/article flex flex-col min-w-0 rounded-lg overflow-hidden bg-[#333233] [html[data-theme=light]_&]:bg-gray-50 hover:bg-[#4a494a] [html[data-theme=light]_&]:hover:bg-gray-100 transition-colors duration-200">
                                                    ...
                                                    <div class="p-4 flex flex-col gap-2">
                                                        <span
                                                            class="text-xs uppercase tracking-widest text-gray-400 [html[data-theme=light]_&]:text-gray-500">{{ $category->name }}</span>
                                                        <p
                                                            class="text-sm font-semibold text-white [html[data-theme=light]_&]:text-gray-900 leading-snug line-clamp-3 group-hover/article:underline decoration-red-primary decoration-2 underline-offset-2 transition-colors duration-200">
                                                            {{ $article->title }}
                                                        </p>
                                                        @if ($article->published_at)
                                                            <span
                                                                class="text-xs text-gray-400 [html[data-theme=light]_&]:text-gray-500">
    ...
    <div x-show="searchOpen" x-transition:enter="transition ease-out duration-200"
        ...
        <div class="max-w-screen-xl mx-auto">
            <form action="{{ route('search') }}" method="GET" class="flex">
                <x-ui.text-input type="text" name="q" placeholder="¿Qué estás buscando?"
                    class="w-full rounded-r-none border-r-0" autofocus />
                <button type="submit"
                    class="bg-red-primary text-white px-4 hover:bg-red-700 transition-colors rounded-l-none rounded-r-md flex items-center cursor-pointer">
    ...
    <div x-show="mobileMenuOpen" class="fixed inset-0 z-50 flex" style="display: none;">
        {{-- Overlay oscuro --}}
        ...
        <div x-show="mobileMenuOpen" x-transition:enter="transition ease-in-out duration-300 transform"
            ...
            <div
                class="px-4 flex items-center justify-between pb-4 border-b border-white/10 [html[data-theme=light]_&]:border-gray-200 transition-colors duration-200">
                <div class="flex items-center gap-3">
                    <img src="{{ asset('images/logos/Isotipo InfoReal Blanco.png') }}" alt="InfoReal isotipo"
                        class="h-8 w-auto [html[data-theme=light]_&]:hidden" width="64" height="64" decoding="async" />
                    <img src="{{ asset('images/logos/Isotipo InfoReal Negro.png') }}" alt="InfoReal isotipo"
                        class="h-8 w-auto hidden [html[data-theme=light]_&]:block" width="64" height="64"
                        decoding="async" />
                    <span
                        class="text-xl font-bold text-white [html[data-theme=light]_&]:text-gray-900 transition-colors duration-200">Categorías</span>
                </div>
                <button @click="mobileMenuOpen = false"
                    class="-mr-2 p-2 text-gray-400 [html[data-theme=light]_&]:text-gray-600 hover:text-white [html[data-theme=light]_&]:hover:text-gray-900 focus:outline-none transition-colors duration-200">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
